...
class OrganizationAggregateTest extends TestCase
{
    ...
    public function testCreateBootstrapsOwnersAndAllMembersTeamWithCreator(): void
    {
        ...
        $organization = Organization::create($id, new Slug('acme'), new DisplayName('ACME Corp'), $ownersTeamId, $allMembersTeamId, self::CREATOR);
    ...
    public function testReconstituteRebuildsStateFromHistory(): void
    {
        $id = new Ulid();
        $ownersTeamId = new Ulid();
        $created = Organization::create($id, new Slug('acme'), new DisplayName('ACME Corp'), $ownersTeamId, new Ulid(), self::CREATOR);
        $history = array_map(
        ...
        self::assertTrue($reloaded->isOwner(self::CREATOR));
        self::assertTrue($reloaded->isOrgMember(self::CREATOR));
    ...
    public function testAllMembersTeamCannotBeRemovedFromManually(): void
    {
        ...
        $this->expectException(TeamProtectedException::class);
        $organization->removeTeamMember($allMembersTeamId, self::CREATOR);
    }
    ...
    {
        $organization = Organization::create(new Ulid(), new Slug('acme'), new DisplayName('ACME Corp'), $ownersTeamId, $allMembersTeamId ?? new Ulid(), self::CREATOR);
        $organization->pullPendingEvents();
    ...
    private function bootstrapHistory(Ulid $ownersTeamId, ?Ulid $allMembersTeamId = null): array
    {
        ...
        return [
            ...
            ['type' => OrganizationEventType::TeamMemberAdded, 'payload' => ['teamId' => $ownersTeamId->toRfc4122(), 'userId' => self::CREATOR]],
            ['type' => OrganizationEventType::TeamMemberAdded, 'payload' => ['teamId' => $allMembersTeamId->toRfc4122(), 'userId' => self::CREATOR]],
        ];
    }
}
