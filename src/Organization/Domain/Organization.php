...
final class Organization extends AbstractAggregate
{
    ...
    public static function create(Ulid $id, Slug $slug, DisplayName $displayName, Ulid $ownersTeamId, int $creatorUserId): self
    {
        $organization = new self($id);
        $organization->record(new OrganizationCreated($id, $slug->value, $displayName->value, $ownersTeamId, $creatorUserId));
